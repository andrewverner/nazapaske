...
class ContentBlock extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, title, text, parent, weight', 'safe', 'on'=>'search'),
		);
	}
    ...
    public static function getBlocksList($parent)
    {
        $cr = new CDbCriteria;
        $cr->condition = 'parent=:parent';
        $cr->params = array(
            ':parent' => $parent
        );
        $cr->order = 'weight ASC';
        return ContentBlock::model()->findAll($cr);
    }
